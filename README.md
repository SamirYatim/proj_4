...
> **Примечание:** В предоставленном коде реализован генератор синтетических данных, имитирующий наличие характерной частоты (сигнала вредителя) на фоне шума. Это позволяет запустить и протестировать проект без необходимости скачивания внешних файлов.
...
*(Здесь предполагается размещение графиков Loss/Accuracy по эпохам, если код будет запущен с сохранением визуализации)*
...
Чтобы использовать реальные данные из Kaggle, внесите изменения в класс `PestAudioDataset` в файле кода:
1. Замените метод `__init__` на загрузку путей к файлам из папки.
2. Реализуйте метод `__getitem__` через `torchaudio.load()`.
3. Передайте путь к вашей папке с данными при инициализации датасета.
